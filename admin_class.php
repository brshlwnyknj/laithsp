...
Class Action {
	...
	function save_house(){
		...
		$data .= ", price = '$price' ";
			...
			if(empty($id)){
				$chk = $this->db->query("SELECT * FROM houses where house_no = '$house_no' AND  category_id = '$category_id' ")->num_rows;
					if($chk > 0 ){
						return 2;
						exit;
					}
				...
				$chk = $this->db->query("SELECT * FROM houses where house_no = '$house_no' AND  category_id = '$category_id' AND id != '$id' ")->num_rows;
				if($chk > 0 ){
					return 2;
					exit;
				}
	...
	function getBootByCategory(){
		...
		if(!empty($_POST["category_id"])){ 
			...
			if($result->num_rows > 0){ 
				echo '<option value="">أختر البوت</option>'; 
				while($row = $result->fetch_assoc()){  
					echo '<option value="'.$row['id'].'">'.$row['house_no'].'</option>'; 
				} 
			}else{ 
				echo '<option value="">غير متوفر بوت</option>'; 
			} 
		}
	}
	...
	function save_in_out(){
		extract($_POST);

		$data = " in_out_no = '$in_out_no' ";
		$data .= ", category_id = '$category_id' ";
		$data .= ", amount = '$amount' ";
		$data .= ", description = '$description' ";
		
			if(empty($id)){
				$chk = $this->db->query("SELECT * FROM in_out where in_out_no = '$in_out_no' AND  category_id = '$category_id' ")->num_rows;
		if($chk > 0 ){
			return 2;
			exit;
		}
				$save = $this->db->query("INSERT INTO in_out set $data");
			}else{

				$chk = $this->db->query("SELECT * FROM in_out where in_out_no = '$in_out_no' AND  category_id = '$category_id' AND id != '$id' ")->num_rows;
		if($chk > 0 ){
			return 2;
			exit;
		}
				$save = $this->db->query("UPDATE in_out set $data where id = $id");
			}
		if($save)
			return 1;
	}
	function delete_in_out(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM in_out where id = ".$id);
		if($delete){
			return 1;
		}
	}
